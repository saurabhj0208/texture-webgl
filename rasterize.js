    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                image = inputTriangles[whichSet].material.texture;  
                lightobj = loadTexture("lightmap.gif");
                if(inputTriangles[whichSet].material.texture!=false)
                { 
                inputTriangles[whichSet].textureObj = loadTexture(image);
                }
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    inputTriangles[whichSet].glTexture.push(1-texToAdd[0],1-texToAdd[1]);
                ...
                gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(inputTriangles[whichSet].glTexture),gl.STATIC_DRAW);
            ...
            //concating the opaque list with non opaque list
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        uniform sampler2D unfLitSampler;
        uniform float unfalpha;
        ...
        void main(void) {
            ...
            vec4 textureColor = texture2D(unfSampler, vec2(vTexture.s, vTexture.t));
            vec4 texture2Color = texture2D(unfLitSampler,vec2(vTexture.s, vTexture.t));

            if(Blinn_Phong && !lightmap && !tex_flag)
            {
                gl_FragColor = vec4(colorOut,unfalpha);
            }
            else if(lightmap && !Blinn_Phong && !tex_flag)
            {
                gl_FragColor = vec4(texture2Color.rgb * colorOut,texture2Color.a * unfalpha) ;
            }
            else if(!Blinn_Phong && !lightmap && !tex_flag)
            {
                gl_FragColor = vec4(colorOut,1.0);
            }
            else if(lightmap && !Blinn_Phong && tex_flag)
            {
                gl_FragColor = vec4(textureColor.rgb * texture2Color.rgb, textureColor.a * texture2Color.a * unfalpha) ;
            }
            else if(Blinn_Phong && !lightmap && tex_flag)
            {
                gl_FragColor = vec4( textureColor.rgb * colorOut, textureColor.a * unfalpha);
            }
            else if(!Blinn_Phong && !lightmap && tex_flag)
            {
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                textureLoc = gl.getAttribLocation(shaderProgram,"aTexture");
                gl.enableVertexAttribArray(textureLoc);

                // locate vertex uniforms
                mMatrixULoc = gl.getUniformLocation(shaderProgram, "umMatrix"); // ptr to mmat
                pvmMatrixULoc = gl.getUniformLocation(shaderProgram, "upvmMatrix"); // ptr to pvmmat
                sampler = gl.getUniformLocation(shaderProgram,"unfSampler");
                litsampler = gl.getUniformLocation(shaderProgram,"unfLitSampler");
                alpha = gl.getUniformLocation(shaderProgram,"unfalpha");
                lighting = gl.getUniformLocation(shaderProgram,"lightmap");
                ...
                texflagULoc = gl.getUniformLocation(shaderProgram,"tex_flag");
...
function loadTexture(image) {
    var path = "https://ncsucgclass.github.io/prog4/" + image;
    var texture = gl.createTexture();
    texture.image = new Image();
    texture.image.onload = function() {
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, texture.image);
        if ((texture.image.width & texture.image.width-1 == 0) && (texture.image.height & texture.image.height == 0)) {
...
function renderModels() {
    ...
    //here storing the order as : OPAQUE FIRST, Sorting the values of rest of the objects
    //according to the Z-buffer values
    var orderofcalling = [];
    var zbufferindex = [];
    for (var whichSet=0; whichSet<numTriangleSets; whichSet++){
        if(inputTriangles[whichSet].material.alpha==1.0)
        {
            orderofcalling.push(whichSet);
        }
        else
        {
            if(zbufferindex.length==0)
            {
                zbufferindex.push(whichSet);
            }
            else
            {
                var i;
                for(i = 0;i<zbufferindex.length;i++)
                {
                	var param = inputTriangles[whichSet].center[2] + inputTriangles[whichSet].translation[2];
                	var param2 = inputTriangles[zbufferindex[i]].center[2] + inputTriangles[zbufferindex[i]].translation[2]
                    if(param>param2)
                        break;
                }
                zbufferindex.splice(i,0,whichSet);
            }
        }
    }
    orderofcalling = orderofcalling.concat(zbufferindex);
    console.log(orderofcalling);
    var currSet; // the tri set and its material properties
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        currSet = inputTriangles[orderofcalling[whichTriSet]];
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER,vertexBuffers[orderofcalling[whichTriSet]]); // activate
        gl.vertexAttribPointer(vPosAttribLoc,3,gl.FLOAT,false,0,0); // feed
        gl.bindBuffer(gl.ARRAY_BUFFER,normalBuffers[orderofcalling[whichTriSet]]); // activate
        gl.vertexAttribPointer(vNormAttribLoc,3,gl.FLOAT,false,0,0); // feed
        gl.bindBuffer(gl.ARRAY_BUFFER,textureBuffers[orderofcalling[whichTriSet]]); // activate
        gl.vertexAttribPointer(textureLoc,2,gl.FLOAT,false,0,0); // feed

        if(inputTriangles[orderofcalling[whichTriSet]].material.texture!=false)
        {
        textureflag = true;
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, inputTriangles[orderofcalling[whichTriSet]].textureObj);
        ...
        gl.uniform1i(texflagULoc,textureflag);
        gl.uniform1f(alpha,currSet.material.alpha);
        gl.uniform1f(lighting,lightm);

        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, lightobj);
        gl.uniform1i(litsampler,1);

        // triangle buffer: activate and render
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,triangleBuffers[orderofcalling[whichTriSet]]); // activate
        gl.drawElements(gl.TRIANGLES,3*triSetSizes[orderofcalling[whichTriSet]],gl.UNSIGNED_SHORT,0); // render
